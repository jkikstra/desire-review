...
# folder `R/raw-data`
POPULATION.PROJECTION.DATA.VERSION <<- "ssp-v301-2024_JUSTMIP"
# POPULATION.PROJECTION.VARIABLE <<- "pop_mil"

# Part 2.3: IAM scenario data ####
IAM.SCENARIO.DATA.FILE <<- "justmip"  #"ENGAGE_SSP2_v4.1.8.3.1_T4.5_r3.1"  #"SHAPE-final_v3_2100_GDPCAP_False_ENSHORT_REF_to_ENLONG_RATIO_2050_thenENLONG_RATIO" # "SHAPE-final_v2" # "CD_LINKS_ssps_downscaled_v3.csv"
...
IAM.OUTPUT.SHORT.FILENAME <<- "JUSTMIP" # "ENGAGE_for_MESSAGE_DLE" # "cdlinks_ssps" # provide a short name for being able to identify output filestrings
IAM.MAPPING.RESCOM.SPLIT.FILENAME <<- "mapping_JUSTMIP_rescomsplits_v1.csv" # for calculator_activity-fe-process-downscaled.R
IAM.MAPPING.DIRTYRES.SPLIT.FILENAME <<- "mapping_JUSTMIP_traditional_biomass_splits_v1.csv" # for calculator_activity-fe-process-downscaled.R
COMMERCIAL.SHARE.TO.KEEP <<- 0.21 # based on US Commercial Buildings Energy Consumption Survey (CBECS) 2018
RES.TRADBIO.SPLIT.METHOD.CHOICE <- "MBuildings-SSP-mapped-for-MESSAGE" # JUSTMIP for MESSAGE DLE sensitivities project v1
...
# Part 2.7: [NOT IN USE YET] climate emulator data ####
CLIMATE.EMULATOR.FILE <<- paste0(IAM.OUTPUT.SHORT.FILENAME, "_temperatures_magicc7.csv")

# Part 2.8: (optional) update DLE threshold data ####
UPDATE.DLE.THRESHOLD <<- T # read in by `calculator_needs-dle-input.R`


# JUSTMIP ####
MESSAGE.REGION.VERSION <- "R12"
ORIGINAL.IAM.DATA.FILE <- "v016_calibrated2025_v2.csv"
