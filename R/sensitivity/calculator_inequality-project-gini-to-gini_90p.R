...
if (GINI.TO.GINI.METHOD == "pure path depency") {
  inequality.projection.i <- inequality.starting.data %>%
    ...
    mutate_cond(gini < gini.start, energy.gini = energy.gini.start + ((gini - gini.start) * (energy.gini.start / gini.start))*0.9) %>% # linear scaling from energy.gini.start to zero.
    mutate_cond(gini > gini.start, energy.gini = energy.gini.start + ((gini - gini.start) * ((100 - energy.gini.start) / (100 - gini.start)))*0.9) %>% # linear scaling from energy.gini.start to one.
